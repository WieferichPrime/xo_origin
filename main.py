...
def training(AI):
    ...
    AI_2.exp_rate = 0.05
    try:
        AI_2.load_policy(f'policy_first_{AI_2.name}.json', f'policy_second_{AI_2.name}.json')
...
def main():
    ...
    AI = Actor('machine')
    try:
        AI.load_policy(f'policy_first_{AI.name}.json', f'policy_second_{AI.name}.json')
    except BaseException as e:
        print(e)
